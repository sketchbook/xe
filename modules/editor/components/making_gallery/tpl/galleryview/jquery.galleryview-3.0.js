...
*/
	...
	*/
		function buildPanels() {
			...
			if(opts.show_panel_nav) {
				$('<span></span>').addClass('panel-nav-next').appendTo(j_gallery).css({
				...
				$('<span></span>').addClass('panel-nav-prev').appendTo(j_gallery).css({
					top:((opts.filmstrip_position=='top'?opts.frame_gap+wrapper_height:0)+(opts.panel_height-22)/2)+'px',
	...
	*/
		function buildFilmstrip() {
			...
			if(filmstrip_orientation=='horizontal') {
				$('.strip_wrapper',j_gallery).css({
					top:opts.show_panels ? opts.filmstrip_position=='top'?0:opts.panel_height+20+'px' : 0,
			...
			if(opts.show_filmstrip_nav) {
				var navNext = $('<span></span>');
				navNext.addClass('nav-next').appendTo(j_gallery).click(showNextItem);
				
				var navPrev = $('<span></span>');
				navPrev.addClass('nav-prev').appendTo(j_gallery).click(showPrevItem);
				
				if(filmstrip_orientation=='horizontal') {
					navNext.css({					 
						top:(opts.show_panels ? (opts.filmstrip_position=='top'?(opts.show_captions?f_caption_height:0):opts.panel_height+20) : 0)+((f_frame_height-16)/2)+'px',
	...
	*/
		return this.each(function() {
			...
			if(filmstrip_orientation=='horizontal') {
				...
				gallery_height = (opts.show_panels?opts.panel_height:0)+(opts.show_filmstrip?wrapper_height:0)+40;
			} else {
				...
				// Height of gallery block = screen + filmstrip + captions (optional)
				gallery_width = (opts.show_panels?opts.panel_width:0)+(opts.show_filmstrip?wrapper_width:0)+40;
			}
...
*/
	$.fn.galleryView.defaults = {
		...
		nav_theme: 'custom',					//STRING - name of navigation theme to use (folder must exist within 'themes' directory)
		...
		frame_gap: 12,						//INT - spacing between frames within filmstrip (in pixels)
