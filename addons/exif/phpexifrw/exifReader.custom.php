...
define("TAG_GPS_IFD" , "0x8825"); // GPS IFD
            ...
            */
            ...
            switch($Tag){
                ...
                case TAG_FNUMBER:
                    ...
                    $this->ImageInfo['h']["fnumber"] = sprintf("%3.1f",(double)$tmp[0]);
                    break;
                ...
                case TAG_MAXAPERTURE:
                    ...
                    if (!isset($this->ImageInfo['h']["aperture"])){
                        ...
                        $this->ImageInfo['h']["aperture"] = sprintf("%3.1f",(double)exp($tmpArr[0]*log(2)/2),1);
                    }
                    break;
                ...
                case TAG_FOCALLENGTH:
                    ...
                    $this->ImageInfo['h']["focalLength"] = sprintf("%d",$tmp[0]);
                ...
                case TAG_EXPOSURETIME:
                    ...
                    // (overwrite previously computd value if there is one)
                    $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                    $this->ImageInfo['h']["exposureTime"] = sprintf("%02.1f",(double)$tmp[0]);
                    if ($tmp[0] <= 0.5){
                ...
                case TAG_EXPOSURE_BIAS:
                    $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                    $this->ImageInfo['h']["exposureBias"] = sprintf("%+.2f",(double)$tmp[0]);
                    break;

                case TAG_LIGHT_SOURCE:
                    $tmp = (int) $this->ConvertAnyFormat($ValuePtr, $Format);
                    $tmpArr = array("1"=>"태양광","2"=>"형광등","3"=>"백열등","4"=>"플래쉬","9"=>"맑은날","10"=>"흐린날");
                    $this->ImageInfo['h']["lightSource"] =
                        (isset($tmpArr["$tmp"]) ? $tmpArr["$tmp"] : "기타");
                    break;

                case TAG_METERING_MODE:
                    $tmp = (int) $this->ConvertAnyFormat($ValuePtr, $Format);

                    $tmpArr = array("1"=>"평균","2"=>"중앙","3"=>"스팟","4"=>"다중스팟","5"=>"다분할");
                ...
                case TAG_EXPOSURE_PROGRAM:
                    $tmp = (int) $this->ConvertAnyFormat($ValuePtr, $Format);
                    $tmpArr = array("1"=>"수동 모드","2"=>"표준 모드","3"=>"조리개 우선 모드","4"=>"셔터 우선 모드","7"=>"인물 모드","8"=>"풍경 모드");
                    $this->ImageInfo['h']["exposure"] =
                        (isset($tmpArr["$tmp"]) ? $tmpArr["$tmp"] : "정보 없음");
                ...
                case TAG_GPS_LATITUDE_REF:
						...
						if($tmp == "S") {
							$this->ImageInfo['h']["GPSLatitudeRef"] = "남위";
						} else if($tmp == "N") {
							$this->ImageInfo['h']["GPSLatitudeRef"] = "북위";
						} else {
							$this->ImageInfo['h']["GPSLatitudeRef"] = "미상";
						}
                        break;
                case TAG_GPS_LATITUDE:
						$degrees = $this->ConvertAnyFormat( substr($ValuePtr,0,8), FMT_URATIONAL);
						$degrees = $degrees[0];
						$minutes = $this->ConvertAnyFormat( substr($ValuePtr,8,8), FMT_URATIONAL);
						$minutes = $minutes[0];
						$seconds = $this->ConvertAnyFormat( substr($ValuePtr,16,8), FMT_URATIONAL);
						$seconds = $seconds[0];
						$this->ImageInfo['h']["GPSLatitude"] = str_replace('.','',"$degrees.도 $minutes.분 $seconds.초");
						$this->ImageInfo['h']["GPSLatitudeUrl"] = round($degrees+$minutes/60+$seconds/3600,6);
                        break;
                ...
                case TAG_GPS_LONGITUDE:
						...
						$seconds = $this->ConvertAnyFormat( substr($ValuePtr,16,8), FMT_URATIONAL);
						$seconds = $seconds[0];
						$this->ImageInfo['h']["GPSLongitude"] = str_replace('.','',"$degrees.도 $minutes.분 $seconds.초");
						$this->ImageInfo['h']["GPSLongitudeUrl"] = round($degrees+$minutes/60+$seconds/3600,6);
                        break;
                ...
                case TAG_GPS_LONGITUDE_REF:
						...
						if($tmp == "E") {
							$this->ImageInfo['h']["GPSLongitudeRef"] = "동경";
						} else if($tmp == "W") {
							$this->ImageInfo['h']["GPSLongitudeRef"] = "서경";
						} else {
							$this->ImageInfo['h']["GPSLongitudeRef"] = "미상";
						}
                        break;
				...
				case TAG_GPS_GPSDestLatitudeRef:/* Reference for latitude of destination  GPSDestLatitudeRef  */
					...
					if($tmp == "S") {
						...
						$this->ImageInfo['h']["GPSDestLatitudeRef"] = "기타";
					}
					break;
				...
				case TAG_GPS_GPSDestLongitudeRef:/* Reference for longitude of destination GPSDestLongitudeRef 21     */
					...
					if($tmp == "E") {
						...
						$this->ImageInfo['h']["GPSDestLongitudeRef"] = "기타";
					}
